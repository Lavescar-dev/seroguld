...
export function PhotosTab({ state }: { state: WooMakeState }) {
  ...
  const detail = state.detail;
  // R1-36 v2 — sürükle-sırala: kalıcı sıra DEDUP ÖNCESİ gerçek id listesiyle
  // PUT edilir; aşağıdaki render dedup'ı yalnız görünüm içindir.
  const realPhotoIds = useMemo(
    () => (detail?.photos || []).map((photo) => photo.id).filter((id): id is string => Boolean(id)),
    [detail?.photos],
  );
  const reorder = usePhotoReorder({
    ids: realPhotoIds,
    disabled: state.isReorderingPhotos,
    onCommit: (orderedIds) => {
      if (detail) state.reorderPhotos(detail.id, orderedIds);
    },
  });
  ...
  return (
    <div
      ...
      onDragOver={(event) => { event.preventDefault(); if (event.dataTransfer.types.includes('Files')) setDragActive(true); }}
