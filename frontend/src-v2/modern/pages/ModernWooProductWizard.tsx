...
export function ModernWooProductWizard({
  ...
  // R1-36 v2 — adım 4 yerel foto sıralama: ağ çağrısı YOK; fotoğraflar
  // onSave(form) ile dizi sırasına göre yüklenir. Obje URL'leri foto
  // başına sabit kalır, yalnız sıra değişir.
  const wizardPhotoReorder = usePhotoReorder({
    ids: form.fotograflar.map((photo) => photo.id),
    onLocalReorder: (orderedIds) => {
      setForm((current) => {
        const byId = new Map(current.fotograflar.map((photo) => [photo.id, photo]));
        const next = orderedIds
          .map((id) => byId.get(id))
          .filter((photo): photo is DraftPhoto => Boolean(photo))
          .map((photo, index) => ({ ...photo, birincil: index === 0 }));
        return next.length === current.fotograflar.length ? { ...current, fotograflar: next } : current;
      });
    },
  });
  ...
  try {
    if (/^https?:/i.test(photo.url)) void openExternalUrl(photo.url);
    else toast.error('Bu önizleme paketlenmiş uygulamada açılamıyor');
  } catch {
    toast.error('Bu önizleme paketlenmiş uygulamada açılamıyor');
  }
}}>Aç</ModernButton><ModernButton aria-label="Fotoğrafı sil" size="sm" tone="danger" icon={Trash2} onClick={() => removePhoto(photo.id)}>Sil</ModernButton></div></div><p className="truncate px-3 py-2 text-xs text-sg-text-soft">{photo.name}</p></div>; })}</div> : <ModernCard className="border-dashed text-center"><ImagePlus className="mx-auto h-7 w-7 text-sg-text-soft" /><p className="mt-2 text-sm text-sg-text-soft">Fotoğraf yüklemek için yukarıdaki düğmeyi kullanın.</p></ModernCard>}{form.wooYayin === 'Yayında' ? <ModernNotice tone="warning" title="Yayın ön koşulları" description="AI açıklaması onaylı olmalı, en az bir fotoğraf ve geçerli shop fiyatı bulunmalı." /> : null}</div> : null}
