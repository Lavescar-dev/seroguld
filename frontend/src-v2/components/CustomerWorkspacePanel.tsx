...
import { formatDate, formatMoney, formatNumber, labelProductType } from '@/lib/format';
...
function transactionMetalLabel(value: string): string {
    ...
    <section className="mt-4 overflow-hidden rounded-sg-lg border border-sg-border bg-sg-surface">
      ...
      <div className="p-4">
        ...
        {tab === 'overview' && workspace ? <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[
          ...
          ['Guld', `${formatNumber(workspace.total_gold_grams)} g`, 'Altın toplamı'],
          ['Sølv', `${formatNumber(workspace.total_silver_grams)} g`, 'Gümüş toplamı'],
          ['Platin', `${formatNumber(workspace.total_platinum_grams)} g`, 'Platin toplamı'],
          ['Palladyum', `${formatNumber(workspace.total_palladium_grams)} g`, 'Palladyum toplamı'],
