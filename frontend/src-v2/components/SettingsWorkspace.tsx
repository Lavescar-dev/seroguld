...
type CategoryKey = 'appearance' | 'market' | 'business' | 'integrations' | 'data';
...
type SettingsWorkspaceProps = {
  ...
  saved: boolean;
  isSaving: boolean;
  // off: bilinçli kapatma — 'Eksik' yerine nötr 'Kapalı' gösterilir.
  apiStatus: Array<{ name: string; ok: boolean; off?: boolean }>;
...
// Firma (fatura satıcı) alanları — backend'de INVOICE_SELLER_* olarak
// kalıcılaşır; mevcut onUpdate/onSave akışını kullanır.
const BUSINESS_FIELDS: FieldDefinition[] = [
  { key: 'firma_adi', label: 'Firma adı', wide: true },
  { key: 'firma_cvr', label: 'CVR numarası', placeholder: '34093083' },
  { key: 'firma_telefon', label: 'Telefon', placeholder: '+45 00 00 00 00' },
  { key: 'firma_email', label: 'E-posta', placeholder: 'info@seroguld.dk' },
  { key: 'firma_adres', label: 'Adres', wide: true },
];
...
const CATEGORIES: Array<{ key: CategoryKey; label: string; description: string; icon: typeof Monitor }> = [
  ...
  { key: 'business', label: 'İşletme bilgileri', description: 'Firma kimliği ve belge bilgileri', icon: Building2 },
...
const CATEGORY_COPY: Record<CategoryKey, { eyebrow: string; title: string; description: string }> = {
  appearance: {
    ...
    description: 'Operatör arayüzünü, dilleri ve müşteri ekranının hangi monitörde açılacağını yönetin.',
  },
  market: {
    eyebrow: 'Fiyatlandırma',
    title: 'Piyasa oranları',
    description: 'Yeni işlemlerde kullanılacak canlı fiyat kaynağını ve kayıtlı oran profilini yönetin.',
  },
  business: {
    eyebrow: 'Firma kimliği',
    title: 'İşletme bilgileri',
    description: 'Fatura satıcı bilgilerini (INVOICE_SELLER_*) backend yapılandırmasından yönetin.',
  },
  integrations: {
    eyebrow: 'Bağlantılar',
    title: 'Entegrasyonlar',
    description: 'Harici servislerin bağlantı bilgilerini ve mevcut yapılandırma durumunu yönetin.',
  },
  data: {
    eyebrow: 'Hesap koruması',
    title: 'Hesap ve güvenlik',
    description: 'Parolanızı, güvenli cihaz kaydını ve yerel verileri yönetin.',
  },
};
...
const INTEGRATIONS: Array<{
  ...
  },
  {
    ...
    label: 'OPMC',
    description:
      'Risk analizi ve sipariş izleme bağlantısı. API anahtarı opsiyonel — modül yapım aşamasında, URL doluysa hazır sayılır.',
    fields: [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'email_transport', label: 'Taşıyıcı', type: 'select', options: EMAIL_TRANSPORT_OPTIONS },
      {
        key: 'wp_bridge_url',
        label: 'Köprü adresi',
        placeholder: 'https://seroguld.dk/wp-json/seroguld/v1/send-afg-email',
        wide: true,
      },
      {
        key: 'wp_bridge_secret',
        label: 'Köprü gizli anahtarı',
        placeholder: '64 haneli hex değer',
        secret: true,
        wide: true,
      },
    ],
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        aria-label={visible ? 'Gizli değeri sakla' : 'Gizli değeri göster'}
      >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 py-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {items.length} kayıt · {formatSize(total)}
          </p>
        </div>
        <button
          type="button"
          onClick={refresh}
          className={`inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold ${classic ? 'border border-brand-300 text-brand-800 hover:bg-brand-50' : 'rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50'}`}
        >
...
function DesktopUpdateSection({ classic }: { classic: boolean }) {
  ...
  useEffect(() => {
    ...
    const readRuntime = () =>
      getDesktopStartupState().then((state) => {
  ...
  const install = async () => {
    ...
    const confirmed = await confirm({
      ...
      message:
        'Kurulum sırasında uygulama kapanır ve yeniden başlar; Excel oturumlarınız kapatılır. Bekleyen kayıtlar önce tamamlanır.',
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-500">
            Uygulama sürümünü denetler. Yeni sürüm indirilir ve kurulur; kurulumdan sonra uygulama kapanıp yeniden
            başlar.
          </p>
          ...
          {update ? (
            <div
              className={`mt-4 p-4 ${classic ? 'border border-brand-200 bg-brand-50/50' : 'rounded-xl border border-slate-200 bg-slate-50'}`}
            >
              <p className="text-sm font-semibold">
                Yeni sürüm: <span className="font-mono">v{update.version}</span>
              </p>
              {update.notes ? (
                <p className="mt-2 whitespace-pre-line text-sm leading-6 text-slate-600">{update.notes}</p>
              ) : null}
              <div className="mt-4">
                <button
                  type="button"
                  onClick={() => void install()}
                  disabled={excelBridgeRunning || installing}
                  className={`inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white transition disabled:cursor-not-allowed disabled:opacity-50 ${classic ? 'bg-brand-800 hover:bg-brand-900' : 'rounded-lg bg-blue-600 hover:bg-blue-700'}`}
                >
...
export function SettingsWorkspace({
  ...
  const integrationStatus = useMemo(
    () => new Map(apiStatus.map((item) => [item.name.toLowerCase(), item])),
    [apiStatus],
  );
  const allConfigured = configuredCount === apiStatus.length && apiStatus.length > 0;

  const pageClass = classic ? 'bg-brand-50/60 text-brand-950' : 'bg-[#f3f6fb] text-slate-950';
  const surfaceClass = classic
    ? 'border-brand-200 bg-white'
    : 'rounded-2xl border-slate-200 bg-white shadow-[0_16px_45px_rgba(15,23,42,0.06)]';
  ...
  const renderField = (field: FieldDefinition) => (
    <label key={String(field.key)} className={field.wide ? 'sm:col-span-2' : undefined}>
      <span
        className={`mb-2 block text-[11px] font-semibold uppercase tracking-[0.16em] ${classic ? 'text-brand-600' : 'text-slate-500'}`}
      >
        {field.label}
      </span>
      {field.secret ? (
        <SecretInput
          ...
          placeholder={
            config.secret_fields_configured?.includes(String(field.key))
              ? 'Yapılandırıldı · değiştirmek için yeni değer girin'
              : field.placeholder
          }
        ...
        <span
          className={`flex items-center justify-between gap-4 px-3.5 py-2.5 ${classic ? '' : 'rounded-xl'} border ${classic ? 'border-brand-200 bg-white' : 'border-slate-200 bg-white'}`}
        >
          <span className={`text-xs ${Boolean(config[field.key]) ? 'text-slate-400' : 'font-semibold text-slate-700'}`}>
            Kapalı
          </span>
          <input
            type="checkbox"
          ...
          />
          <span className={`text-xs ${Boolean(config[field.key]) ? 'font-semibold text-blue-700' : 'text-slate-400'}`}>
            Açık
          </span>
        </span>
      ) : (
    ...
    <nav className="space-y-1" aria-label="Ayar kategorileri">
      {CATEGORIES.map((item) => {
        ...
        return (
          ...
          >
            <span
              className={`flex h-9 w-9 shrink-0 items-center justify-center ${active ? (classic ? 'bg-brand-700 text-white' : 'rounded-lg bg-blue-600 text-white') : classic ? 'border border-brand-200 bg-white text-brand-600' : 'rounded-lg border border-slate-200 bg-white text-slate-500'}`}
            >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span
                className={`mt-0.5 block truncate text-xs ${active ? (classic ? 'text-brand-600' : 'text-blue-700') : 'text-slate-400'}`}
              >
                {item.description}
              </span>
            </span>
            <ChevronRight
              className={`h-4 w-4 shrink-0 transition ${active ? 'opacity-100' : 'opacity-0 group-hover:opacity-50'}`}
            />
          </button>
        );
      })}
    ...
    <div className={`min-h-full ${pageClass}`}>
      <div className="mx-auto max-w-[1480px] px-4 py-5 sm:px-6 lg:px-8">
        <header className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p
              className={`text-[11px] font-semibold uppercase tracking-[0.22em] ${classic ? 'text-brand-600' : 'text-blue-600'}`}
            >
              Sistem yapılandırması
            </p>
            <h1 className="mt-1 text-2xl font-semibold tracking-[-0.025em]">Ayarlar</h1>
            <p className="mt-1 text-sm text-slate-500">
              İşletme, ekran ve bağlantı tercihlerini tek çalışma alanında yönetin.
            </p>
          </div>
          <div
            className={`inline-flex w-fit items-center gap-2 px-3 py-2 text-sm ${classic ? 'border border-brand-200 bg-white' : 'rounded-xl border border-slate-200 bg-white shadow-sm'}`}
          >
            <span className={`h-2 w-2 rounded-full ${allConfigured ? 'bg-emerald-500' : 'bg-amber-500'}`} />
            <span className="font-semibold">
              {configuredCount}/{apiStatus.length}
            </span>
        ...
        <div className="mb-4 lg:hidden">
          <select
            ...
            className={inputClass}
            aria-label="Ayar kategorisi seç"
          >
            {CATEGORIES.map((item) => (
        ...
        <div className="grid items-start gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
          ...
          <main className={`min-w-0 border ${surfaceClass}`}>
            <div className={`border-b px-5 py-5 sm:px-7 ${classic ? 'border-brand-200' : 'border-slate-200'}`}>
              <p
                className={`text-[10px] font-semibold uppercase tracking-[0.2em] ${classic ? 'text-brand-600' : 'text-blue-600'}`}
              >
                {activeCopy.eyebrow}
              </p>
            ...
            <div className="px-5 py-6 sm:px-7">
              {category === 'appearance' ? (
                <div className="space-y-8">
                  ...
                  <section
                    className={`border-t pt-7 [&>section]:!border-0 [&>section]:!bg-transparent [&>section]:!p-0 [&>section]:!shadow-none ${classic ? 'border-brand-200' : 'border-slate-200'}`}
                  >
                    {languageSlot}
                  </section>
                  <section
                    className={`border-t pt-7 [&>*]:!border-0 [&>*]:!bg-transparent [&>*]:!p-0 [&>*]:!shadow-none ${classic ? 'border-brand-200' : 'border-slate-200'}`}
                  >
                    {monitorSlot}
                  </section>
                </div>
              ...
              {category === 'market' ? (
                <div className="max-w-5xl space-y-7">
                  <div
                    className={`p-4 ${classic ? 'border border-brand-200 bg-brand-50' : 'rounded-xl border border-slate-200 bg-slate-50'}`}
                  >
                    <p className="text-sm font-semibold">Otomatik oran alanları</p>
                    <p className="mt-1 text-sm leading-6 text-slate-500">
                      Yalnız bu üç alan istenirse canlı kaynaktan (Stooq) otomatik çekilebilir; her biri{' '}
                      <strong>bağımsızdır</strong> (hepsi bir arada olmak zorunda değil). Altın ve gümüş karat oranları{' '}
                      <strong>WP "Priser" sayfasından</strong> çekilir veya üst çubuktaki oran editöründen elle girilir.
                    </p>
                    {(() => {
                      const AUTO_FIELDS = [
                      ...
                      return (
                        <div
                          className={`mt-4 space-y-1 border-t pt-3 ${classic ? 'border-brand-200' : 'border-slate-200'}`}
                        >
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="inline-flex items-center gap-2 text-xs">
                                  <span className={auto ? 'text-slate-400' : 'font-semibold text-slate-700'}>
                                    Manuel
                                  </span>
                                  <input
                                    type="checkbox"
                                    checked={auto}
                                    onChange={(event) => setAutoField(key, event.target.checked)}
                                    className="h-4 w-4 accent-blue-600"
                                  />
                                  <span className={auto ? 'font-semibold text-blue-700' : 'text-slate-400'}>
                                    Oto · canlı kur
                                  </span>
                                </span>
                              </label>
                          ...
                          <p className="mt-2 text-xs leading-5 text-slate-500">
                            Manuelde bırakılan alan, oran editöründeki değerinde kalır.
                          </p>
                        </div>
                      );
                    })()}
                  ...
                  <div
                    className={`p-4 ${classic ? 'border border-emerald-300 bg-emerald-50' : 'rounded-xl border border-emerald-200 bg-emerald-50'}`}
                  >
                    <p className="text-sm font-semibold text-emerald-800">
                      WP "Priser" sayfası — tek karat/gümüş kaynağı
                    </p>
                    <p className="mt-1 text-sm leading-6 text-emerald-700">
                      Karat ve gümüş oranları seroguld.dk "Priser" sayfasından çekilir (üst çubuk → Au/Ag →{' '}
                      <strong>WP'den çek</strong>). Elle ezilen değerler bir sonraki çekime kadar korunur.
                    </p>
                    <p className="mt-2 text-xs font-semibold text-emerald-800">
                      Son çekim:{' '}
                      {config.wp_priser_last_fetch
                        ? new Date(config.wp_priser_last_fetch).toLocaleString('da-DK')
                        : 'henüz çekilmedi'}
                    </p>
                  </div>
                  <section>
                    ...
                    <div
                      className={`mt-3 divide-y ${classic ? 'divide-brand-200 border-y border-brand-200' : 'divide-slate-200 border-y border-slate-200'}`}
                    >
                      {[
                    ...
                    <p className="mt-4 text-sm leading-6 text-slate-500">
                      Karat ve gümüş saflıklarının ayrıntılı değerlerini üst çubuktaki{' '}
                      <strong className="font-semibold text-slate-700">Au / Ag</strong> düğmesinden düzenleyin.
                    </p>
                  </section>
                </div>
              ) : null}
              ...
              {category === 'business' ? (
                <div className="max-w-4xl space-y-5">
                  <p className="max-w-3xl text-sm leading-6 text-slate-500">
                    Bu alanlar backend'de fatura satıcı bilgisi olarak saklanır ve{' '}
                    <strong className="font-semibold text-slate-700">Değişiklikleri kaydet</strong> ile kalıcılaşır.
                  </p>
                  <div className="grid gap-x-5 gap-y-5 sm:grid-cols-2">{BUSINESS_FIELDS.map(renderField)}</div>
                </div>
              ) : null}
              ...
              {category === 'integrations' ? (
                <div className="space-y-3">
                  ...
                  {INTEGRATIONS.map((item) => {
                    ...
                    // Üç durum: yapılandırıldı / bilinçli kapalı / eksik. Bilinçli
                    // kapatma nötrdür; amber 'Eksik' yalnız gerçek eksikliktedir.
                    const state = status?.ok ? 'ready' : status?.off ? 'off' : 'missing';
                    const badgeTone =
                      state === 'ready'
                    ...
                    return (
                      <section
                        key={item.key}
                        className={
                          classic ? 'border border-brand-200 bg-white' : 'rounded-xl border border-slate-200 bg-white'
                        }
                      >
                        <button
                          type="button"
                        ...
                        >
                          <span className="flex min-w-0 items-center gap-3">
                            <span
                              className={`h-2 w-2 shrink-0 rounded-full ${state === 'ready' ? 'bg-emerald-400' : state === 'off' ? 'bg-slate-300' : 'bg-amber-400'}`}
                            />
                          ...
                          <span className="flex shrink-0 items-center gap-2">
                            <span
                              className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold ${badgeTone} ${classic ? 'border border-current/20' : 'rounded-full'}`}
                            >
                              {state === 'ready' ? (
                              ...
                              {state === 'ready' ? 'Yapılandırıldı' : state === 'off' ? 'Kapalı' : 'Eksik'}
                            </span>
                        ...
                        {open ? (
                          <div
                            className={`border-t px-4 pb-5 pt-4 ${classic ? 'border-brand-200' : 'border-slate-200'}`}
                          >
                            <div className="grid max-w-4xl gap-x-5 gap-y-5 sm:grid-cols-2">
              ...
              {category === 'data' ? (
                <div className="max-w-5xl space-y-8">
                  <section className="max-w-3xl">
                    <PasswordChangeForm variant={variant} />
                  </section>
                  ...
                  <section>
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-500">
                      Bu cihazın yapılandırmasını dosya olarak dışa aktarabilir veya daha önce alınmış bir ayar
                      dosyasını geri yükleyebilirsiniz.
                    </p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={onImport}
                        className={`inline-flex items-center gap-2 px-3.5 py-2.5 text-sm font-semibold ${classic ? 'border border-brand-300 text-brand-800 hover:bg-brand-50' : 'rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-50'}`}
                      >
                  ...
                  <section
                    className={`flex items-start gap-3 border-t pt-6 ${classic ? 'border-brand-200' : 'border-slate-200'}`}
                  >
                    <Shield className="mt-0.5 h-5 w-5 shrink-0 text-emerald-600" />
                    <div>
                      <p className="text-sm font-semibold">Gizli değerler korunur</p>
                      <p className="mt-1 text-sm leading-6 text-slate-500">
                        API anahtarları backend üzerinden saklanır. Boş bırakılan gizli alanlar mevcut kayıtlı değeri
                        silmez.
                      </p>
                    </div>
                  </section>
                  <section className={`border-t pt-7 ${classic ? 'border-brand-200' : 'border-slate-200'}`}>
                    ...
                    <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
                      {t('auth.security.description', getLocale())}
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                        const email = getCurrentUser()?.email || '';
                        void deleteStoredLoginPassword(email).then((removed) =>
                          setForgetPasswordStatus(removed ? 'saved' : 'failed'),
                        );
                      }}
                    ...
                    {forgetPasswordStatus === 'saved' ? (
                      <p role="status" className="mt-2 text-sm text-emerald-700">
                        {t('auth.forgetPassword.saved', getLocale())}
                      </p>
                    ) : null}
                    {forgetPasswordStatus === 'failed' ? (
                      <p role="alert" className="mt-2 text-sm text-rose-700">
                        {t('auth.forgetPassword.failed', getLocale())}
                      </p>
                    ) : null}
                  </section>
                </div>
        ...
        <div
          className={`sticky bottom-3 z-sticky mt-5 flex flex-col gap-3 border px-4 py-3 shadow-[0_14px_40px_rgba(15,23,42,0.12)] sm:flex-row sm:items-center sm:justify-between ${classic ? 'border-brand-300 bg-white' : 'rounded-2xl border-slate-200 bg-white/95 backdrop-blur'}`}
        >
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className={saved ? 'font-semibold text-emerald-700' : 'text-slate-500'}>
              {isSaving
                ? 'Değişiklikler kaydediliyor'
                : saved
                  ? 'Değişiklikler kaydedildi'
                  : 'Ayarlar kaydedilmeye hazır'}
            </span>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              onClick={onReset}
              disabled={isSaving}
              className={`inline-flex items-center gap-2 px-3.5 py-2.5 text-sm font-semibold transition disabled:opacity-50 ${classic ? 'border border-brand-300 text-brand-700 hover:bg-brand-50' : 'rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50'}`}
            >
            ...
            <button
              ...
              onClick={onSave}
              disabled={isSaving}
              className={`inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white transition disabled:opacity-70 ${classic ? 'bg-brand-800 hover:bg-brand-900' : 'rounded-lg bg-blue-600 hover:bg-blue-700'}`}
            >
