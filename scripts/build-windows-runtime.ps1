...
function Get-CustomerRuntimeSeedAllowedKeys {
  ...
  return @(
    ...
    "EMAIL_TRANSPORT", "WP_BRIDGE_URL", "WP_BRIDGE_SECRET", "AFG_EMAIL_ENABLED",
...
function Invoke-SourceGit {
  ...
  # PS 5.1 turns native stderr (e.g. an autocrlf warning) into an error
  # record even under 2>$null, which is fatal under EAP=Stop.  The 2>$null
  # already intends to discard stderr; $LASTEXITCODE still catches real
  # git failures below.
...
function Test-SourceFingerprintPath {
  ...
  # `tauri build` regenerates gen/schemas on every run; on a fresh checkout
  # (or after a tauri upgrade) the regenerated bytes differ from the tracked
  # ones, so the file list/hash would change between manifest creation and
  # the release-time verification.  Generated schemas are not release inputs.
